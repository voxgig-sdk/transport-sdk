...
type Args struct {
	Entity string         `json:"entity" jsonschema:"connection | location | stationboard"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.TransportSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "transport_list",
		Description: "List records from Transport. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "transport_load",
		Description: "Load a single record from Transport. " +
...
func runOp(client *sdk.TransportSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.TransportSDK, name string) (sdk.TransportEntity, error) {
	switch strings.ToLower(name) {
	case "connection":
		return client.Connection(nil), nil
	case "location":
		return client.Location(nil), nil
	case "stationboard":
		return client.Stationboard(nil), nil
