...
[MCP](https://modelcontextprotocol.io) server exposing the Transport SDK as
two agent tools — `transport_list` and `transport_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/transport-mcp)
make build

# 2. Provide credentials via the environment
export TRANSPORT_APIKEY=sk_live_xxx
...
```jsonc
// transport_list: first page of records
{ "entity": "connection" }
{ "entity": "connection", "query": { } }

// transport_load: one record by id
{ "entity": "connection", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/transport-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user transport \
     -- "$PWD"/dist/*/transport-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `transport_list` and `transport_load` tools now appear
   in new sessions. Ask the agent to *"list connection using transport"*
   and it calls `transport_list` with `{"entity":"connection"}`.
...
```sh
export TRANSPORT_APIKEY=sk_live_xxx            # API key
export TRANSPORT_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `transport_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `transport_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `TRANSPORT_APIKEY` | API key sent with every request. |
| `TRANSPORT_BASE` | Optional override of the API base URL. |
