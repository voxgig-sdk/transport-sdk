...
The Python SDK for the Transport API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Connection(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.Transport(None).load(
    {"id": "test01"}, None
)
