...
class TestLocationDirect:

    def test_should_direct_list_location(self):
        setup = _location_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-location", "live" if setup["live"] else "unit")
        if _skip:
...
def _location_direct_setup(mockres):
