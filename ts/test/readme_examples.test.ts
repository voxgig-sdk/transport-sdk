...
//     (\`new TransportSDK(...)\` / \`TransportSDK.test(...)\` become
//     \`TransportSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"connection":{"test01":{"id":"test01"}},"location":{"test01":{"id":"test01"}},"stationboard":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new TransportSDK(...)\` and any
// \`TransportSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+TransportSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/TransportSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
