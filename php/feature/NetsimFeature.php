...
class TransportNetsimFeature extends TransportBaseFeature
{
    ...
    private function _simulate(TransportContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(TransportContext $ctx, array $applied): void
    {
        $client = $this->client;
