...
// Transport SDK Stationboard entity
...
class StationboardEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new StationboardEntity($this->_client, $opts);
    }

    /**
     * @param Stationboard|array $args Stationboard data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = TransportHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Stationboard|array The current Stationboard data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Stationboard fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = TransportHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Stationboard fields).
     */
    ...
    /**
     * List Stationboard items matching the given filter.
     *
     * @param StationboardListMatch|array|null $reqmatch Match filter (any subset
     *   of Stationboard fields) as an assoc-array; StationboardListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Stationboard[]|array A list of Stationboard items as assoc-arrays at
     *   the SDK boundary; throws TransportError on failure (item-5 convention).
     */
