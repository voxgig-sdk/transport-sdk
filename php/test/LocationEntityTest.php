...
class LocationEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Location(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Location(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = location_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "location." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set TRANSPORT_TEST_LOCATION_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $location_ref01_data = null;
        if (count($location_ref01_data_raw) > 0) {
...
function location_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/location/LocationTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["location01", "location02", "location03"] as $k) {
    ...
    $entid_env_raw = getenv("TRANSPORT_TEST_LOCATION_ENTID");
