...
class FtCtrl extends TransportControl
{
...
class FtClient
{
    ...
    private TransportUtility $utility;

    public function __construct(TransportUtility $utility, array $options)
    {
    ...
    public function get_utility(): TransportUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (TransportContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public TransportContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (TransportContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = TransportConfig::make_feature($name);
    ...
    private static function buildUrl(TransportSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof TransportError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new TransportResponse($fetched);
        }
    ...
    private function fail(TransportContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (TransportContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
