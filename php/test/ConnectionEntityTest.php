...
class ConnectionEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set TRANSPORT_TEST_CONNECTION_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $connection_ref01_data = null;
        if (count($connection_ref01_data_raw) > 0) {
        ...
        [$connection_ref01_list_result, $err] = $connection_ref01_ent->list($connection_ref01_match, null);
        $this->assertNull($err);
...
function connection_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["connection01", "connection02", "connection03"] as $k) {
    ...
    $entid_env_raw = getenv("TRANSPORT_TEST_CONNECTION_ENTID");
