...
{
    private static function utility(): TransportUtility
    {
        return new TransportUtility();
    }
    ...
    private static function ctx(array $over = []): TransportContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new TransportContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new TransportOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof TransportError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new TransportOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = TransportMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new TransportOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = TransportMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new TransportOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = TransportMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new TransportSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = TransportMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        ...
        [, $err] = TransportMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new TransportSpec([]),
            'response' => null,
            'result' => new TransportResult([]),
        ]);
        [, $err] = TransportMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new TransportSpec(['step' => 's']),
            'response' => new TransportResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new TransportResult([]),
        ]);
        [, $err] = TransportMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new TransportSpec(['step' => 's']),
            'response' => new TransportResponse(self::resp(200, ['v' => 1])),
            'result' => new TransportResult([]),
        ]);
        [, $err] = TransportMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new TransportSpec(['step' => 's']),
            'response' => new TransportResponse(self::resp(200, ['v' => 2])),
            'result' => new TransportResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        TransportMakeResponse::call($ctx);
    ...
    {
        $preset = new TransportResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new TransportSpec([]),
            'response' => new TransportResponse([]),
            'result' => new TransportResult([]),
        ...
        [$got, $err] = TransportMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new TransportResult([])]);
        [, $err] = TransportMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new TransportSpec([]), 'result' => null]);
        [, $err] = TransportMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new TransportSpec(['step' => 's']),
            'result' => new TransportResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new TransportOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = TransportMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new TransportSpec(['step' => 's']),
            'result' => new TransportResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new TransportOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = TransportMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new TransportResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new TransportSpec([]),
            'result' => new TransportResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = TransportMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = TransportMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new TransportError('boom', 'boom');
        $utility->fetcher = function (TransportContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new TransportSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = TransportMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (TransportContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (TransportContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new TransportSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = TransportMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new TransportResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new TransportSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = TransportMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = TransportMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = TransportMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new TransportResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, TransportDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new TransportResult(['ok' => false])]);
        $this->expectException(TransportError::class);
        TransportDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new TransportResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', TransportMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        TransportMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new TransportBaseFeature();
        $b = new TransportBaseFeature();
        TransportFeatureAdd::call($ctx, $a);
        TransportFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): TransportBaseFeature
    {
        $f = new TransportBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        TransportFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): TransportContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new TransportSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = TransportPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new TransportResponse(['headers' => null]),
            'result' => new TransportResult([]),
        ]);
        TransportResultHeaders::call($ctx);
