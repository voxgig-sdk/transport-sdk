...
class TransportSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = TransportHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = TransportHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!TransportFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, TransportFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return TransportUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = TransportHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = TransportHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = TransportHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new TransportError($op . "_allow",
                "TransportSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = TransportHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new TransportError("graphql_error",
                "TransportSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    private $_stationboard = null;

    // Canonical facade: $client->Stationboard()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->stationboard()
    // resolves here too.
    public function Stationboard($data = null)
    {
        require_once __DIR__ . '/entity/stationboard_entity.php';
        if ($data === null) {
            if ($this->_stationboard === null) {
                $this->_stationboard = new StationboardEntity($this, null);
            }
            return $this->_stationboard;
        }
        return new StationboardEntity($this, $data);
    }
