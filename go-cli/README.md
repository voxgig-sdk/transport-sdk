...
boru-driven command-line client **and** interactive REPL for the Transport
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/transport-cli)
make build
...
./transport-cli list connection
./transport-cli list location

# 5. Override the API base URL for a single call
TRANSPORT_BASE=https://api.example.com ./transport-cli list connection

# 6. No arguments -> interactive REPL
./transport-cli
transport> list connection
transport> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/transport-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/transport-cli list connection
   ```
...
Run with no arguments to open a REPL (prompt `transport>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `connection`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/transport-cli`. |
...
(`./transport-cli <expr>`) and interactively (the REPL), and expressions compose the
