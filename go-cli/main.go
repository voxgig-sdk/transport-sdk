// Command Transport-cli is a CLI + REPL for the Transport SDK,
...
const entitiesHelp = "connection location stationboard"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      TRANSPORT_APIKEY (api key), TRANSPORT_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: TRANSPORT_APIKEY carries the API key and
	// TRANSPORT_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("TRANSPORT_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("TRANSPORT_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewTransportSDK(opts)
