...
class ConnectionDirectTest < Minitest::Test
  def test_direct_list_connection
    setup = connection_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-connection", setup[:live] ? "live" : "unit")
    if _should_skip
...
def connection_direct_setup(mockres)
