...
class LocationDirectTest < Minitest::Test
  def test_direct_list_location
    setup = location_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-location", setup[:live] ? "live" : "unit")
    if _should_skip
...
def location_direct_setup(mockres)
