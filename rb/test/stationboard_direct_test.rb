...
class StationboardDirectTest < Minitest::Test
  def test_direct_list_stationboard
    setup = stationboard_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-stationboard", setup[:live] ? "live" : "unit")
    if _should_skip
...
def stationboard_direct_setup(mockres)
