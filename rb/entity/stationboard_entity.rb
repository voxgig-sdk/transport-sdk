...
class StationboardEntity
  ...
  def make
    opts = @_entopts.dup
    StationboardEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = TransportHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Stationboard, Hash] the current Stationboard data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = TransportHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Stationboard fields)
  def match_get
  ...
  #
  # @param reqmatch [StationboardListMatch, Hash, nil] match filter (any subset of
  #   Stationboard fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Stationboard>, Array] the matching Stationboard items; raises TransportError on failure
