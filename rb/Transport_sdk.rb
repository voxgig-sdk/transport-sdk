...
require_relative 'Transport_types'
...
class TransportSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = TransportHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = TransportHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, TransportFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    TransportUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = TransportHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = TransportHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = TransportHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = TransportHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = TransportError.new(
        "graphql_error", "TransportSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
