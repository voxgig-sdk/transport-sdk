...
import (
	...
	omni "github.com/voxgig-sdk/transport-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.TransportError); is {
				out := map[string]any{
					"name":    "TransportError",
...
func omniCtx(arg any, client *sdk.TransportSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.TransportSDK); is {
			client = live
