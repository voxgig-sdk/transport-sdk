...
## Install
```bash
go get github.com/voxgig-sdk/transport-sdk/go@latest
```

The Go module proxy resolves the version from the `go/vX.Y.Z` GitHub
release tag — see [Releases](https://github.com/voxgig-sdk/transport-sdk/releases) for the available versions.
...
```bash
go mod edit -replace github.com/voxgig-sdk/transport-sdk/go=../transport-sdk/go
```
...
func main() {
    ...
    // List connection records — the value is the array of records itself.
    connections, err := client.Connection(nil).List(nil, nil)
    if err != nil {
        panic(err)
    }
    for _, item := range connections.([]any) {
        fmt.Println(item)
    }
}
```
...
```go
func TestSDK(testopts map[string]any, sdkopts map[string]any) *TransportSDK
```
...
| `Connection` | `(data map[string]any) TransportEntity` | Create a Connection entity instance. |
| `Location` | `(data map[string]any) TransportEntity` | Create a Location entity instance. |
| `Stationboard` | `(data map[string]any) TransportEntity` | Create a Stationboard entity instance. |

### Entity interface (TransportEntity)

All entities implement the `TransportEntity` interface.
...
Create an instance: `stationboard := client.Stationboard(nil)`
...
```go
stationboards, err := client.Stationboard(nil).List(nil, nil)
if err != nil {
    panic(err)
}
fmt.Println(stationboards) // the array of records
```
...
```
github.com/voxgig-sdk/transport-sdk/go/
├── transport.go        # Root package — type aliases and constructors
...
The root package (`github.com/voxgig-sdk/transport-sdk/go`) re-exports everything needed
