...
Learn more about Transport API at [transport.opendata.ch](http://transport.opendata.ch).
...
This SDK exposes the API as a small set of **semantic entities** — Connection, Location and Stationboard — that you
...
const items = await client.Connection().list({ from: "example", to: "example" })
```
...
```go
client := sdk.Test()
result, err := client.Connection(nil).List(
    nil, nil,
)
```
...
$connections = $client->Connection()->list();
print_r(array_map(fn($item) => $item->data_get(), $connections));
```
