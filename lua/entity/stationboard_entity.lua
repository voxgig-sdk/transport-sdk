...
local StationboardEntity = {}
StationboardEntity.__index = StationboardEntity


function StationboardEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, StationboardEntity)
  self._name = "stationboard"
  self._client = client
...
function StationboardEntity:get_name()
  return self._name
end


function StationboardEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return StationboardEntity.new(self._client, opts)
end
...
function StationboardEntity:mark_deleted()
  self._deleted = true
end


function StationboardEntity:deleted()
  return true == self._deleted
end


function StationboardEntity:data_set(args)
  if args ~= nil then
...
function StationboardEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function StationboardEntity:match_set(args)
  if args ~= nil then
...
function StationboardEntity:match_get()
...
function StationboardEntity:stream(action, args, callopts)
...
---@param reqmatch StationboardListMatch
---@param ctrl? table
---@return Stationboard[]
---@return string? err
function StationboardEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function StationboardEntity:_run_op(ctx, post_done)
  local utility = self._utility
