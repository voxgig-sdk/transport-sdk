...
LocationEntity.__index = LocationEntity


function LocationEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, LocationEntity)
  self._name = "location"
  self._client = client
...
function LocationEntity:get_name()
  return self._name
end


function LocationEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return LocationEntity.new(self._client, opts)
end
...
function LocationEntity:mark_deleted()
  self._deleted = true
end


function LocationEntity:deleted()
  return true == self._deleted
end


function LocationEntity:data_set(args)
  if args ~= nil then
...
function LocationEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function LocationEntity:match_set(args)
  if args ~= nil then
...
function LocationEntity:match_get()
...
function LocationEntity:stream(action, args, callopts)
...
function LocationEntity:_run_op(ctx, post_done)
  local utility = self._utility
