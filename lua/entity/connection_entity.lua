...
ConnectionEntity.__index = ConnectionEntity


function ConnectionEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ConnectionEntity)
...
function ConnectionEntity:get_name()
  return self._name
end


function ConnectionEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ConnectionEntity.new(self._client, opts)
end
...
function ConnectionEntity:mark_deleted()
  self._deleted = true
end


function ConnectionEntity:deleted()
  return true == self._deleted
end


function ConnectionEntity:data_set(args)
  if args ~= nil then
...
function ConnectionEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ConnectionEntity:match_set(args)
  if args ~= nil then
...
function ConnectionEntity:match_get()
...
function ConnectionEntity:stream(action, args, callopts)
...
function ConnectionEntity:_run_op(ctx, post_done)
  local utility = self._utility
