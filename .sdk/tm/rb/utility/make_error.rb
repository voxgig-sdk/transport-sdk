...
module TransportUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = TransportContext.new({}, nil)
    end
    op = ctx.op || TransportOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || TransportResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(TransportError) ? err.msg : err.to_s
    ...
    sdk_err = TransportError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(TransportError)
