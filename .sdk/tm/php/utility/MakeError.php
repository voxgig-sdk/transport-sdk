...
class TransportMakeError
{
    public static function call(?TransportContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new TransportContext([], null);
        }
        $op = $ctx->op ?? new TransportOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new TransportResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof TransportError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new TransportError('', $msg, $ctx);
